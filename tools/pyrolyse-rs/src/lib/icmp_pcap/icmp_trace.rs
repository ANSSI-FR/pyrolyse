...
#[derive(Debug, Clone)]
pub struct IcmpTraceD<I> {
    v: Vec<IcmpPacket<I>>,
}

impl<I: Debug + Copy + IpAddrGeneric> IcmpTraceD<I> {
    pub fn new(v: Vec<IcmpPacket<I>>) -> IcmpTraceD<I> {
        IcmpTraceD { v }
    }
    ...
    pub fn get(&self, index: usize) -> Option<&IcmpPacket<I>> {
        self.v.get(index)
    }
    ...
    pub fn icmp_packet_v_of_data(
        ...
        let chunk_data_hm = match policy_evaluation {
            ...
            PolicyEvaluation::OnceStartPrecedes => {
                let mut hm = chunk_c
                    .iter()
                    .map(|(i, chunk_d)| {
                        let chunk_index = chunk_d.get_index();
            ...
            PolicyEvaluation::OnceStartFollows => {
                let mut hm = chunk_c
                    .iter()
                    .map(|(i, chunk_d)| {
                        let chunk_index = chunk_d.get_index();
            ...
            PolicyEvaluation::NewProgressive => {
                let hm = chunk_c
                    .iter()
                    .map(|(i, chunk_d)| {
                        let chunk_index = chunk_d.get_index();
                        ...
                        let data = if offset != 0 {
                            ...
                            vec![icmp_header.to_vec(),chunk_d.get_internet_checksum_ascii_v().clone()].concat()
                        };
        ...
        let mut index_position_ethernet_data_v = chunk_data_hm
            ...
            .collect::<Vec<(u16, u16, IcmpPacket<I>)>>();
    ...
    pub fn of_data_byte_time_sequence_d<Rc: Clone + Serialize + RelationContainer>(
        test_target: &TestTarget<I>,
        policy_evaluation: &PolicyEvaluation,
        icmp_header: &[u8],
        test_index: u16,
    ...
    pub fn export(
        ...
        self.v.iter().enumerate().for_each(|(i, icmp_packet)| {
            ...
            let data = icmp_packet.build_ethernet_pdu_data_v();
...
#[derive(Debug, Clone)]
pub struct IcmpTraceC<I> {
    hm: HashMap<u16, IcmpTraceD<I>>,
}

impl<I: IpAddrGeneric> FromIterator<(u16, IcmpTraceD<I>)> for IcmpTraceC<I> {
    fn from_iter<U>(iter: U) -> Self
    where
        U: IntoIterator<Item = (u16, IcmpTraceD<I>)>,
    {
        Self {
...
impl<I: Debug + Copy + IpAddrGeneric> IcmpTraceC<I> {
    pub fn len(&self) -> usize {
        self.hm.len()
    }
    ...
    pub fn get(&self, index: &u16) -> Option<&IcmpTraceD<I>> {
        self.hm.get(index)
    }

    pub fn iter(&self) -> Iter<u16, IcmpTraceD<I>> {
        self.hm.iter()
    }

    /// Build an ICMP header.
    pub fn build_icmp_header(
        policy_evaluation: &PolicyEvaluation,
        ...
        let chunk_nb_for_checksum = match policy_evaluation {
            ...
            PolicyEvaluation::NewProgressive => total_length,
            PolicyEvaluation::OnceStartPrecedes => total_length,
            PolicyEvaluation::OnceStartFollows => total_length,
        ...
        let icmp_pdu_data_v =
            I::build_icmp_pdu_data_v(test_index, sequence_index, &data, &ip_src, &ip_dst);
    ...
    pub fn of_data_byte_time_pair_sequence_c(
        ...
        let map = tuple_v
            ...
            .map(|(test_index, byte_time_sequence_d)| {
                ...
                (
                    **test_index,
                    IcmpTraceD::of_data_byte_time_sequence_d(
                        test_target,
                        policy_evaluation,
                        &icmp_header,
                        **test_index,
