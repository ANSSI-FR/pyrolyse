...
use crate::udp_pcap::udp_packet::UdpPacket;

// TODO: factorize with IcmpPacket

#[derive(Debug, Clone)]
pub struct UdpTraceD<I> {
    v: Vec<UdpPacket<I>>,
}

impl<I: Debug + Copy + IpAddrGeneric> UdpTraceD<I> {
    pub fn new(v: Vec<UdpPacket<I>>) -> UdpTraceD<I> {
        UdpTraceD { v }
    }
    ...
    pub fn get(&self, index: usize) -> Option<&UdpPacket<I>> {
        self.v.get(index)
    }
    ...
    pub fn udp_packet_v_of_data(
        ...
        debug!("udp_packet_v_of_data: start");
        ...
        let chunk_data_hm = match policy_evaluation {
            ...
            PolicyEvaluation::OnceEnd => {
                ...
                // debug!(
                //     "udp_packet_v_of_data: ending_chunk_offset: {}",
                //     ending_chunk_offset
                // );
                ...
                let starting_chunk_index = chunk_nb;
                let starting_chunk_temporal_position = 0;
                let starting_chunk_payload = udp_header.to_vec();
        ...
        let mut index_position_udp_packet_v = chunk_data_hm
            .iter()
            .map(
                |(_, (chunk_index, temporal_position, more_chunk, offset, data))| {
                    // .map(|chunk_d| {
                    // debug!("udp_packet_v_of_data: chunk_d: {:?}", chunk_d);
                    debug!("udp_packet_v_of_data: chunk_index: {:?}", chunk_index);

                    // let chunk_index = chunk_d.get_index();
                    // let temporal_position = temporal_position_sl.get(chunk_index as usize).unwrap();

                    debug!(
                        "udp_packet_v_of_data: chunk_d data ({:?}): {:?}",
                        data.len() / 8,
                        data
                    );
                    debug!(
                        "udp_packet_v_of_data: chunk_d data hex ({:?}): {:x?}",
                        data.len() / 8,
                        data
                    );
            ...
            .collect::<Vec<(u16, u16, UdpPacket<I>)>>();

        debug!(
            "udp_packet_v_of_data: index_position_udp_packet_v: {:?}",
            index_position_udp_packet_v
        );
        index_position_udp_packet_v.sort_by_key(|tuple| {
        ...
        let udp_packet_v = index_position_udp_packet_v
            .into_iter()
            .map(|(_, _, icmp_packet)| icmp_packet)
            .collect();

        debug!("udp_packet_v_of_data: end");
    ...
    //     let icmp_packet_v = UdpTraceD::udp_packet_v_of_data(
    //         test_target,
    //         policy_evaluation,
    //         temporal_position_v,
    //         icmp_header,
    ...
    //     // UdpTrace::new(index_icmp_packet_v)
    //     UdpTraceD::new(icmp_packet_v)
    // }
    ...
    pub fn export(
        ...
        self.v.iter().enumerate().for_each(|(i, udp_packet)| {
            let ts_sec = i as u32;
            let ts_usec = 0;

            let data = udp_packet.build_ethernet_pdu_data_v();
...
#[derive(Debug, Clone)]
pub struct UdpTraceC<I> {
    hm: HashMap<u16, UdpTraceD<I>>,
}

impl<I: IpAddrGeneric> FromIterator<(u16, UdpTraceD<I>)> for UdpTraceC<I> {
    fn from_iter<U>(iter: U) -> Self
    where
        U: IntoIterator<Item = (u16, UdpTraceD<I>)>,
    {
        Self {
...
impl<I: Debug + Copy + IpAddrGeneric> UdpTraceC<I> {
    pub fn len(&self) -> usize {
        self.hm.len()
    }
    ...
    pub fn get(&self, index: &u16) -> Option<&UdpTraceD<I>> {
        self.hm.get(index)
    }

    pub fn iter(&self) -> Iter<u16, UdpTraceD<I>> {
        self.hm.iter()
    }
    ...
    pub fn build_udp_header(
        ...
        let udp_pdu_data_v = I::build_udp_pdu_data_v(test_index, &data, &ip_src, &ip_dst);
        ...
        let icmp_header = &udp_pdu_data_v[0..8].to_vec();
    ...
    pub fn of_data_byte_time_pair_sequence_c(
        ...
        let map = tuple_v
            .iter()
            .map(|(test_index, byte_time_sequence_d)| {
                debug!("of_data: sequence_index: {}", test_index);

                let icmp_header = UdpTraceC::build_udp_header(
                    policy_evaluation,
                ...
                (
                    **test_index,
                    UdpTraceD::of_data_byte_time_sequence_d(
                        test_target,
                        policy_evaluation,
                        &icmp_header,
                        **test_index,
