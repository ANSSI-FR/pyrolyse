...
use rst_lib::minimal_policy::all_policy::AllPolicy;
...
fn main() -> io::Result<()> {
    ...
    let matches = Command::new("extract-reassembly-policies")
        ...
        .arg(
            Arg::new("payload-json-path")
                .long("payload-json-path")
                .help("Payload JSON path")
                .required(true),
        )
        .arg(
            Arg::new("policy-json-path")
                .long("policy-json-path")
    ...
    // let policy_json_path_s = match matches.value_of("policy-json-path") {
    let policy_json_path_s = match matches.get_one::<String>("policy-json-path") {
        Some(s) => s,
        None => {
            eprintln!("No policy JSON path provided");
            exit(-1);
        }
    };
    println!("policy_json_path_s: {}", policy_json_path_s);
    let policy_json_path = Path::new(policy_json_path_s);
    ...
    let all_policy = AllPolicy::of_data(
        &payload_mode,
    ...
    let json_string: String = serde_json::to_string_pretty(&all_policy).unwrap();
    fs::write(policy_json_path, json_string).expect("Unable to write file");
