...
//use rst_lib::full_policy::all_position_policy::AllPositionPolicy;
...
fn main() -> io::Result<()> {
    ...
    let matches = Command::new("extract-reassembly-policies")
        ...
        .arg(
            Arg::new("full-policy-json-path")
                .long("full-policy-json-path")
                .help("full policy JSON path")
                .required(true),
        )
        .arg(
    ...
    // let full_policy_json_path_s = match matches.value_of("full-policy-json-path") {
    let full_policy_json_path_s = match matches.get_one::<String>("full-policy-json-path") {
        Some(s) => s,
        None => {
            eprintln!("No full policy JSON path provided");
            exit(-1);
        }
    };
    println!("full_policy_json_path_s: {}", full_policy_json_path_s);
    let full_policy_json_path = Path::new(full_policy_json_path_s);
    ...
    let all_position_policy = rst_lib::ip_full_policy::all_policy::AllPolicy::of_data(
        &payload_mode,
    ...
    fs::write(full_policy_json_path, json_string).expect("Unable to write file");
