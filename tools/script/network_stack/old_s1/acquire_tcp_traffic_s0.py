...
class Sniffer(Thread):
    ...
    def update_last_data(self, received_packet):
        ...
        if ip_packet.src == self.destination_ip:
            print(
                f"Sniffer: update_last_data: {ip_packet.src} -> {ip_packet.dst} : {tcp_flags} - S={tcp_pdu.seq-self.tcb_irs} A={tcp_pdu.ack-self.tcb_iss}"
            )
        else:
            print(
                f"Sniffer: update_last_data: {ip_packet.src} -> {ip_packet.dst} : {tcp_flags} - S={tcp_pdu.seq-self.tcb_iss} A={tcp_pdu.ack-self.tcb_irs}"
            )
        ...
        if received_packet[
            ...
            print(f"Sniffer: update_last_data: time: {received_packet.time-self.initial_time}")
    ...
    def send_ack(self, received_packet):
        ...
        if ip_packet.src == self.destination_ip:
            print(
                f"Sniffer: send_ack: {ip_packet.src} -> {ip_packet.dst} : {tcp_flags} - S={tcp_pdu.seq-self.tcb_irs} A={tcp_pdu.ack-self.tcb_iss}"
            )
        else:
            print(
                f"Sniffer: send_ack: {ip_packet.src} -> {ip_packet.dst} : {tcp_flags} - S={tcp_pdu.seq-self.tcb_iss} A={tcp_pdu.ack-self.tcb_irs}"
            )
        ...
        if ip_packet.src == self.destination_ip and received_packet[
            ...
            print(f"Sniffer: send_ack: time: {received_packet.time-self.initial_time}")
...
class SingleTest(Thread):
    ...
    def send_message(self):
        ...
        print(
            f"SingleTest: run: tcp_pdu.seq: {tcp_pdu.seq}"
        )
        self.tcb_snd_una = synack.ack
        print(
            f"SingleTest: run: tcp_pdu.ack: {tcp_pdu.ack}"
        )
