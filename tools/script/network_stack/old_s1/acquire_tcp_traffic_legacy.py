...
class Sniffer(Thread):
    ...
    def __init__(self, sp, dip, rstack, myack, scenario, interface="eth1"):
        super().__init__()
    ...
    def keep_echo_reply_with_max_ack(self, packet):
        ...
        print(
            'Sniffer: keep_echo_reply_with_max_ack: packet[IP].src == self.dip: ',
            packet[IP].src == self.dip)
        print(
            'Sniffer: keep_echo_reply_with_max_ack: packet[TCP].dport == self.sport: ',
            packet[TCP].dport == self.sport)
        print('Sniffer: keep_echo_reply_with_max_ack: packet[TCP].ack: ',
              packet[TCP].ack)
        print('Sniffer: keep_echo_reply_with_max_ack: self.myack: ',
              self.myack)

        if packet[IP].src == self.dip and packet.haslayer(
                TCP) and packet[TCP].dport == self.sport and packet[
                    TCP].flags == 0x18 and packet[TCP].ack >= self.myack:
    ...
    def send_ack(self, packet):
        ...
        if ip_layer.src == self.dip and packet[
                TCP].dport == self.sport and packet[TCP].flags == 0x18:
            ...
            tcp = TCP(ack=packet[TCP].seq + len(b),
...
class SingleTest(Thread):
    ...
    def __init__(self, test_index, sp, dp, sip, dip, scenario, offset, payload,
    ...
    def run(self):
        ...
        p = subprocess.Popen([
            ...
            f"host {self.dip} and port {self.sp}"
        ],
    ...
    def sendMSG(self):
        ISN = 10
        tcp = TCP(dport=self.dp, sport=self.sp, flags="S", seq=ISN)
        syn = self.ip / tcp
        synack = sr1(syn)
    ...
    def send_pep2_sequence(self):
        for i in range(len(self.offset)):
            ...
            sniffer = Sniffer(self.sp, self.dip, self.rstack, self.myack,
                              self.scenario)
    ...
    def send_pep1_sequence(self):
        ...
        sniffer = Sniffer(self.sp, self.dip, self.rstack, 0, self.scenario)
        print("[*] Start sniffing...")
        sniffer.start()
    ...
    def sendRST(self):
        print("sendRST: self.tcpseq + self.max_offset_before_any_hole: ",
              self.tcpseq + self.max_offset_before_any_hole)
        print("sendRST: max(self.rstack): ", max(self.rstack))
        LASTRST = self.ip / TCP(
            sport=self.sp,
            dport=self.dp,
            flags="RA",
            seq=self.tcpseq + self.max_offset_before_any_hole,
            ack=max(self.rstack))
        send(LASTRST)
...
def Process(test_index, sip, dip, dp, byte_time_sequence_json_path,
    ...
    singleTest = SingleTest(test_index, sp, dp, sip, dip, scenario, offset,
...
def main():
    ...
    Process(test_index, sip, dip, dp, byte_time_sequence_json_path,
            output_pcap_path, scenario)
