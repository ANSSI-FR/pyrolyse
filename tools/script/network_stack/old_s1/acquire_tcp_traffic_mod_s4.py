...
# Even if located after a hole, a sequence number is however still valid because it is inside the send window.
TCB_SND_NXT = 0
...
class SingleTest(Thread):
    ...
    def send_pep2_sequence(self):
        ...
        for i, _ in enumerate(self.offset):
            # Send payload
            seg_seq = self.tcb_snd_nxt_after_handshake + self.offset[i]
            tcp = TCP(dport=self.destination_port,
                      sport=self.source_port,
                      ack=self.myack,
                      seq=seg_seq,
            ...
            TCB_SND_NXT = max(TCB_SND_NXT,seg_seq)
    ...
    def send_pep1_sequence(self):
        ...
        for i, _ in enumerate(self.offset):
            ...
            print(f"[*] SingleTest: send_pep1_sequence: TCB_SND_NXT: {TCB_SND_NXT}")
    ...
    def send_peos_sequence(self):
        ...
        for i, _ in enumerate(self.offset):
            # Send payload
            time.sleep(0.2)
            seg_seq=self.tcb_snd_nxt_after_handshake + self.offset[i] + 1
            tcp = TCP(dport=self.destination_port,
                      sport=self.source_port,
                      seq=seg_seq,
                      ack=self.myack,
