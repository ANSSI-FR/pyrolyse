...
class Sniffer(Thread):
    ...
    def keep_echo_reply_with_max_ack(self, received_packet):
        ...
        print("Sniffer: keep_echo_reply_with_max_ack: packet['TCP'].ack: {received_packet['TCP'].ack}")
    ...
    def send_ack(self, received_packet):
        ...
        print("Sniffer: send_ack: packet['TCP'].dport: {received_packet['TCP'].dport}")
        print("Sniffer: send_ack: self.source_port: {self.source_port}")
        print("Sniffer: send_ack: packet['TCP'].flags == 0x18: {received_packet['TCP'].flags == 0x18}")

        print("Sniffer: send_ack: self.index: {self.index}")
        print("Sniffer: send_ack: self.rstack: {self.rstack}")
...
class SingleTest(Thread):
    ...
    def run(self):
        ...
        print(f"[*] SingleTest: run: end capturing traffic for test of index {self.test_index}")
    ...
    def send_message(self):
        ...
        if self.scenario == "peos":
            ...
            print("SingleTest: run: the scenario {self.scenario} doesn't exist")
            sys.exit(-1)
    ...
    def send_rst(self):
        ...
        Sends a RST.
        """
        print("SingleTest: send_rst: self.tcb_tcp_seq + self.max_offset_before_any_hole: ",
              self.tcb_tcp_seq + self.max_offset_before_any_hole)
        print("SingleTest: send_rst: max(self.rstack): ",
              max(self.rstack))
...
def get_max_offset_before_any_hole(offset_l: list[int], payload_l: list[str]):
    ...
    print("get_max_offset_before_any_hole: payload_offset_d: ", payload_offset_d)
