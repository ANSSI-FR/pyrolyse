...
class Sniffer(Thread):
    ...
    def send_ack(self, packet):
        ...
        if ip_layer.src == dip and packet[TCP].dport == self.sport and packet[TCP].flags == 0x18:
          ...
          self.rstack[self.index] = packet[TCP].seq + len(packet[TCP].payload)
          self.rstseq[self.index] = packet[TCP].ack
          ip=IP(src=ip_layer.dst, dst=ip_layer.src)
          tcp=TCP(ack=packet[TCP].seq + len(packet[TCP].payload), dport=packet[TCP].sport, sport=self.sport, flags="A",seq=packet[TCP].ack) #window=5480
          ackit=ip/tcp
          send(ackit)
...
def sendMSG(offset, payload):
    ...
    ISN =10
    #ether=Ether(dst='6e:2d:27:45:c8:82')
...
def ExecuteData(dicts, curr_time):
    ...
    for i in dict_no:
        ...
        #paylen = dicts['hm'][i]['payload_byte_length']

        #   Get the offset
        #   offset = list([d['offset'] for d in slice_data])  -- Obsolete line
        offset = []
        ...
        # Get the payload
        # payload = list([d['internet_checksum_s'] for d in slice_data]) -- Obsolete line
        payload = []
        ...
        # index = list([d['index'] for d in slice_data]) -- Obsolete line
        index = []
        ...
        time.sleep(0.5)
        #print("Multiple offset %s \n Payload %s Payload Length %s" % (multiplied_offset, payload, paylen))
...
def main():
    ...
    curr_time = os_directory + "/TCP_segmentation_scenario_2_" + datetime.now().strftime("%Y-%m-%d_%H:%M")
