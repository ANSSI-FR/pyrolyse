...
def main(argv):
    ...
    parser.add_argument("-v", "--ip-version", type=str, default="4")
    ...
    print(
        f"extract_os_icmp_payload_from_directory_to_json: ip_version: {ip_version}"
    )
    print(
        f"extract_os_icmp_payload_from_directory_to_json: json_path: {json_path}"
    )

    process(pcap_directory, ip_version, json_path,
            nb_final_character_to_remove)
