...
function replay_single_pcap_copy_log() {
    ...
    if [ "$protocol" = "ipv4" ] || [ "$protocol" = "ipv6" ]; then
        signature_file=$conf_dir/icmp_overlap_first_chunk_piece.sig
    elif [ "$protocol" = "tcp" ]; then
        signature_file=$conf_dir/tcp_overlap_all_chunks.sig
    fi
    ...
    echo "replay_single_pcap_copy_log: handling signatures.log"
    if [ -f $zeek_log_tmp_directory_path/signatures.log ]; then
        mv $zeek_log_tmp_directory_path/signatures.log $log_path
    else
