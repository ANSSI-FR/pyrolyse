...
def extract_data(index_data_offset_d, test_case_index, log_path, nb_final_character_to_remove):
    ...
    for log in content:
        if "payload" in log.keys():
            payload = log["payload_printable"] if nb_final_character_to_remove == 0 else log["payload_printable"][:-nb_final_character_to_remove]
            print("extract_data: is_echo_reply: True")
            print("extract_data: payload: ",payload)
