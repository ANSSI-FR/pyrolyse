...
def extract_tcp_payload_from_script(
    ...
    print("extract_tcp_payload_from_script: tcp_payload_b_len:",tcp_payload_b_len)
    ...
    print("extract_tcp_payload_from_script: payload_no_starting_extra_char_b: ",payload_no_starting_extra_char_b)
    ...
    print("extract_tcp_payload_from_script: tc_theorical_ending_byte_offset: ",tc_theorical_ending_byte_offset)
    print("extract_tcp_payload_from_script: nb_final_character_to_remove: ",nb_final_character_to_remove)
    ...
    print("extract_tcp_payload_from_script: payload_no_extra_char_b: ",payload_no_extra_char_b)
...
#    test_case_index:int,
#):
#    # TODO Add signature tcp payload extraction
...
#        print("extract_tcp_payload_v: line: ",str(line))
#
#        # For script
#        if "tcp::Tcp_content_From_Client" in line:
#            zeek_msg_s = line.split("\t")[11]
#            zeek_msg_d = ast.literal_eval(zeek_msg_s)
#            current_tcp_seq_s = zeek_msg_d["seq"]
#            current_tcp_payload_s = zeek_msg_d["contents"]
#
...
#            print('extract_tcp_payload_v: TODO Add signature tcp payload extraction')
#            exit(-1)
#        
#        tcp_payload_len_approx_i += len(current_tcp_payload_s)
#        
...
#            tcp_seq_num_payload_hex_d.update( { int(current_tcp_seq_s): new_entry } )
#    
#    print("extract_tcp_payload_v: tcp_seq_num_payload_hex_d: ", tcp_seq_num_payload_hex_d)
#    
#    # non-ascii characters are hex char (e.g. \xff) 
#    tcp_seq_num_payload_b_d = { }
#    for tcp_seq,tcp_payload_hex_s_v in tcp_seq_num_payload_hex_d.items():
#        print("extract_tcp_payload_v: tcp_payload_hex_s_v: ",tcp_payload_hex_s_v)
#        tcp_payload_b_s_v = [ ]
#        for tcp_payload_hex_s in tcp_payload_hex_s_v:
#            print("extract_tcp_payload_v: tcp_payload_hex_s: ",tcp_payload_hex_s)
#            reconstructed_payload_b = b""
#            curr_pos = 0
#            # replace non-printable char 
#            while curr_pos < len(tcp_payload_hex_s):
...
#                    reconstructed_payload_b += bytes(tcp_payload_hex_s[curr_pos], 'utf-8')
#                    curr_pos += 1
#            print("extract_tcp_payload_v: reconstructed_payload_b: ",reconstructed_payload_b)
#            tcp_payload_b_s_v.append(reconstructed_payload_b)
#        tcp_seq_num_payload_b_d.update({ tcp_seq: tcp_payload_b_s_v})
#    
#    # Concatenate "sub-payloads" and adding "." substring if there is a hole
...
#        if len(payload_v) == 1:
#            tmp_tcp_payload_b = tmp_tcp_payload_b[:seq - 1] + payload_v[0] + tmp_tcp_payload_b[seq + len(payload_v[0]) - 1:]
#            print("extract_tcp_payload_v: tmp_tcp_payload_s", tmp_tcp_payload_b)
#        else:
#            # different reassembled payload (because different data for same seq number)  
#            return { "is_echo_reply": True, "number": len(payload_v), "payload": "" }
#    tcp_payload_b = tmp_tcp_payload_b[:seq + len(payload_v[0]) - 1]
#
#    tcp_payload_b_len = len(reconstructed_payload_b)
...
#    ## we then ensure that we can remove ending extra bytes
#    tc_theorical_ending_byte_offset = get_tc_theorical_ending_byte_offset(test_case_index,byte_time_sequence_hm)
#    print("extract_tcp_payload_v: tc_theorical_ending_byte_offset: ",tc_theorical_ending_byte_offset)
#    print("extract_tcp_payload_v: nb_final_character_to_remove: ",nb_final_character_to_remove)
#    if nb_final_character_to_remove == 0 or tc_theorical_ending_byte_offset + nb_final_character_to_remove > len(payload_no_starting_extra_char_b):
#        payload_no_extra_char_b = payload_no_starting_extra_char_b 
#    else:
#        payload_no_extra_char_b = payload_no_starting_extra_char_b[:-nb_final_character_to_remove]
...
def extract_tcp_payload_v(
    ...
    if "tcp::Tcp_content_From_Client" in line_l_w_payload_v[0]:
        tcp_payload_b = extract_tcp_payload_from_script(line_l_w_payload_v,byte_time_sequence_hm,nb_starting_character_to_remove,nb_final_character_to_remove,test_case_index)
...
def extract_ip_payload_v(
    ...
    for line_w_payload in line_w_payload_v:
        ...
        ## we then ensure that we can remove ending extra bytes
        tc_theorical_ending_byte_offset = get_tc_theorical_ending_byte_offset(test_case_index,byte_time_sequence_hm)
        print("extract_ip_payload_v: tc_theorical_ending_byte_offset: ",tc_theorical_ending_byte_offset)
        print("extract_ip_payload_v: nb_final_character_to_remove: ",nb_final_character_to_remove)
        if nb_final_character_to_remove == 0 or tc_theorical_ending_byte_offset + nb_final_character_to_remove > len(payload_no_starting_extra_char_b):
            payload_no_extra_char_b = payload_no_starting_extra_char_b 
        else:
            payload_no_extra_char_b = payload_no_starting_extra_char_b[:-nb_final_character_to_remove]
...
def main(argv):
    ...
    print('extract_payload_from_zeek_log_directory_to_json: merged_byte_time_sequence_json_path: "%s"'%(merged_byte_time_sequence_json_path))
