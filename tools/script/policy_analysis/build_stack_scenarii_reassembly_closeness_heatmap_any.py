...
def build_target_name_v_from_csv_data(csv_data_v: list,
                                      target_to_filter: list) -> list:
    ...
    target_name_v = [
        ...
        if target_name not in target_to_filter
    ]
...
def sort_scenario_v_by_agnostic_dependant(scenario_v: list) -> list:
    #agnostic_dependant_hm = { scenario.split('-')[0]:(scenario.split('-')[1] if len(scenario.split('-')) == 2 else "") for scenario in scenario_v }

    agnostic_only = [
        scenario for scenario in scenario_v if len(scenario.split('-')) == 1
    ]
    dependant_only = [
        scenario for scenario in scenario_v if len(scenario.split('-')) == 2
    ]

    return agnostic_only + dependant_only


def process(input_csv_filepath: str, output_plot_filepath: str,
            target_to_filter: list):
    print("process: start")
    ...
    scenario_v_sorted_by_agnostic_dependant = sort_scenario_v_by_agnostic_dependant(
        scenario_v)
    print("process: scenario_v_sorted_by_agnostic_dependant: ",
          scenario_v_sorted_by_agnostic_dependant)
    target_name_v = build_target_name_v_from_csv_data(csv_data_v,
                                                      target_to_filter)
    print("process: target_name_v: ", target_name_v)

    corr_matrix = build_custom_corr_matrix_hm(
        csv_data_v, scenario_v_sorted_by_agnostic_dependant, target_name_v)
    ...
    scenario_dependant_part_v = [
        get_scenario_dependant_part_from_pyrolyse_scenario_name_format(
            scenario) for scenario in scenario_v_sorted_by_agnostic_dependant
    ]
...
def main(argv):
    ...
    target_to_filter = args.target_to_filter
    ...
    print(
        f"build_stack_scenarii_reassembly_closeness_heatmap: target_to_filter: {target_to_filter}"
    )

    process(input_csv_filepath, output_plot_filepath, target_to_filter)
