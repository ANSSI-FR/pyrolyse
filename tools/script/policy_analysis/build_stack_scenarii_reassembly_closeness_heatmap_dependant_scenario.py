...
mpl.rcParams[
    'text.latex.preamble'] = r"""\usepackage{bm}"""  # to get bold math tex text title
...
def get_scenario_agnostic_part_from_pyrolyse_scenario_name_format_with_s(
    ...
    return r'$\bm{s^{%s}}$' % scenario_name_agnostic_part_modified
...
def is_agnostic_scenario_part_in_scenario(agnostic_scenario_part: str,
                                          scenario_to_test: str) -> bool:
    scenario_to_test_agnostic_part = scenario_to_test.split('-')[0]
    return scenario_to_test_agnostic_part == agnostic_scenario_part
...
def build_scenario_v_from_csv_data(csv_data_v: list,
    ...
    scenario_v = [
        ...
        and scenario.split('-')[0] == agnostic_scenario_part
    ]
...
def process(input_csv_filepath: str, output_plot_filepath: str,
    ...
    #cbb.set_label('Implementation number', labelpad=-25)
...
def main(argv):
    ...
    parser.add_argument("-s",
                        "--agnostic-scenario-part",
                        choices=["pep", "peosp", "peosf"],
                        type=str)
    ...
    print(
        f"build_stack_scenarii_reassembly_closeness_heatmap: agnostic_scenario_part: {agnostic_scenario_part}"
    )
    process(input_csv_filepath, output_plot_filepath, agnostic_scenario_part)
